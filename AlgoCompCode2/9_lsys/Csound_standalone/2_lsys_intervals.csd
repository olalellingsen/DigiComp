...
instr 2
  itempo = p4 
  ktrig metro itempo
  knote init p5 ; initialize pitch, it will be modified by the intervals (from the L-system) later
  ...
  if ktrig > 0 then ; when the metronome ticks...
	...
	knote += giIntervals[0] ; read the interval from the global array and modify our note value
	...
	kspecial = 1 ;we can do something special to set each generation apart from the others
      endif
      if kspecial != 1 then ; in this example, we use a pause between each generation
	kamp = -5
	ipan = 0.5
	kdur = 1/itempo
