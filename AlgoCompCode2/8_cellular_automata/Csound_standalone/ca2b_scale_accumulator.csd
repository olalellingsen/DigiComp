...
instr 10
  ...
  if ktrig == 1 then
    ...
    ; calculate the note by using the value in gkIntervals as an index into a meldic scale (in the giScale array),
    ; .. and change the octave (add 12 semitones) when we try to index past the end (size of) of the scale
    knote = ibasenote + giScale[gkIntervals[kcount]%lenarray(giScale)] + int(gkIntervals[kcount]/lenarray(giScale))*12
    if kcount == 0 then
      kamp = -5
    else
      kamp = -12
